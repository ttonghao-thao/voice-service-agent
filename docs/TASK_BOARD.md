...
更新：2026-09-24。目标设计见 [architecture.md](architecture.md)，状态证据见 [acceptance-report.md](acceptance-report.md)。本期范围已确定为英文知识库客服，默认只启用 `search_knowledge`。

## 1. 当前结论与审计基线

当前代码以 D01–D06、D08–D10、E01–E03 的本地验证为基础；D12 已把验证入口收敛为服务端固定 customer、同源 Web/API 和 15 项运行配置，D09/D10 的账号登录和私网 API 属于已被本期新要求替换的历史实现。D07 尚未完成真实环境验收。2026-09-22 的 `9b44859` 是历史文档审计基线，验证日期和范围见 [验收 §0–2](acceptance-report.md#0-当前审计与证据索引)。
...
| D02 | 身份与知识权限。`api/auth.py`、API、历史/引用读取 | 验证身份不具备管理权限；tenant/KB 由服务端确定，正文不能覆盖 | 授权和范围复核逻辑完成；D12 将部署入口收敛为固定 customer，正式多客户认证后续设计；真实 CueKB ACL 待验收 |
...
| D07 | 云端 Docker 真实端到端、故障、性能与上线。部署及测试模块 | 固定验证身份的英文语音→CueKB→实际口述正确；PostgreSQL/Redis/容器与长会话通过；记录版本、样本和阈值 | 部署前检查、Compose 与英文探针代码完成；云端真实验证待执行 |
...
D12 已完成验证配置、固定身份、门户和部署入口收敛并通过本地回归。D07 的真实服务验收仍待执行。Q 系列是审计提出的增量设计，尚未编码或安排，不把它们自动追加为原有编码交付前提。
...
| P0 / D07-A | 发布基线与云端环境，待执行 | 固定版本/配置/样本，按公网 HTTPS Web 的单一流程验收；迁移、readiness 与恢复证据齐全 | [部署 D07](deployment.md#d07-分阶段执行设计) |
| P0 / D07-B | 固定验证身份、真实文本模型与 CueKB，待执行 | 服务端 KB 范围/管理拒绝、M3 命中/故障/限额和有依据回答通过 | 同上；V02–V04、V10 |
...
| P0 / D12 | 功能验证部署配置收敛，本地完成 | 固定 customer/tenant/KB、移除登录/JWT和宿主 API 端口；固定模式移入 Compose，两个镜像共用一个标签；VoiceChat 证据写探针报告，不用运行开关阻塞启动。保持全双工、Coordinator、Agent、ToolRegistry 与数据边界；73 项 pytest、前端测试/构建及静态检查通过 | [部署](deployment.md#配置与受限测试身份)、[验收 D12](acceptance-report.md#2026-09-24-d12-功能验证配置收敛) |
...
D12 只收敛验证入口和配置，不修改 VoiceGateway、SessionCoordinator、BusinessRuntime、ToolRegistry、CueKB adapter、revision/epoch、单写入器或 pending call 结清语义。本地夹具测试不记为真实服务验收。下一步按 D07-A–D 执行端到端全双工验证；云端实测缺陷回到原模块修复并复测。
