...
更新：2026-09-24。**D01–D06、D08–D10、E01–E03 已完成历史编码和本地自动化验证，D12 已完成功能验证入口收敛；D07 的真实服务端到端验收仍未完成**。本期仅验收英文知识库客服；真实验证在云端 Docker 环境执行。缺口和实施顺序见 [任务板](TASK_BOARD.md)。
...
| 要回答的问题 | 证据位置与边界 |
| --- | --- |
| 最近应用验证 | §1 的 D12：73 项 pytest、Ruff、前端音频测试/构建、契约导出和部署脚本语法；不代表真实部署 |
