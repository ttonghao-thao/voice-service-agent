...
更新：2026-09-23。**D01–D06、D08–D10、E01–E03 已完成代码和本地自动化验证，D07 的真实服务与生产验收仍未完成**。D11 整理当前文档与发布记录。本期仅验收英文知识库客服；真实验证在云端 Docker 环境执行。缺口和实施顺序见 [任务板](TASK_BOARD.md)。
...
| 要回答的问题 | 证据位置与边界 |
| --- | --- |
| 最近应用验证 | §1 的 D10：75 项 pytest、Ruff 和部署脚本语法；前端音频测试与构建见 D09，不代表真实部署 |
...
- 受限测试账号在真实 HTTPS 门户的登录、过期与撤权传播，以及真实文本模型。对外客户身份接入不在本期；天气/股票不在本期验收范围，保留实现未做真实供应商验收。
