...
中文语音能力由目标 VoiceChat 服务提供，本项目负责中文无损传输和端到端验证，不训练模型、不管理语音 GPU 栈。默认按单组织部署，客户可见知识范围由服务端决定；不宣称已有跨组织完整多租户能力。
...
- 当前 React/Ant Design 工作台是待简化的已有实现，不是最终客户页面。后续按交互范围精简，避免先重写音频底层。
...
目标语义需后续实现；当前 interrupt 仍是清音、任务取消、epoch 失效及关闭连接的组合操作，不应在门户把它标成仅停止播报。
...
若增强门槛未通过，只能声明基础语音客服可用，不能把增加子 agent 当作完整业务全双工已实现。当前 105 秒连接上限是应用策略；目标在安静边界受控轮换并保留确认过的业务摘要，不承诺恢复模型隐藏状态或重播旧音频。
...
已有会话、音频、SDK、工具注册和存储骨架，可增量复用。D01 已冻结门户事件契约和语音能力声明/探测边界，D02 已实现客户角色与知识历史撤权保护。主要缺口：CueKB 原生适配、简单门户、独立任务版本及停止播报语义、按启用工具校验配置、真实身份/知识/语音能力验收。

按 [任务板](TASK_BOARD.md) 的 D01–D07 执行；D01/D02 的本地代码阶段已完成，下一步实现业务适配和页面，最后在真实环境验证并部署。
