    ...
    async def stream(self, ws, sid, ticket):
        ...
        def emit(kind, payload):
            ...
            try:
                ...
                raise DomainError("AUDIO_BACKPRESSURE", "播放数据积压，请重新开始语音", 409, True) from exc
            ...
            async for event in provider.events():
                ...
                if event.kind == "tool":
                    ...
                    if call in pending:
                        continue
                    if len(pending) >= 128:
                        raise DomainError("VOICE_PROTOCOL_ERROR", "语音工具请求过多", 502)
                    if any(v in ("running", "ready") for v in pending.values()):
                        raise DomainError("VOICE_PROTOCOL_ERROR", "云端并行工具调用不受支持，请重新提问", 502)
        ...
        async def watchdog():
            while True:
                ...
                if session.principal.expires_at is not None and session.principal.expires_at <= time.time():
                    raise DomainError("AUTH_REQUIRED", "登录已过期，请重新登录", 401)
                if not await current():
                    return
        ...
        try:
            ...
            error = DomainError("VOICE_UNAVAILABLE", "语音连接已断开，请重新开始或使用文字", 503, True)
        finally:
